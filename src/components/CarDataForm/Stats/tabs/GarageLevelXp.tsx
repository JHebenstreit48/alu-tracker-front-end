...
export default function GarageLevelXp({ fields, noCarsSelected, carSelector, perCarNote }: Props) {
  if (noCarsSelected) return <EmptyState message="Select a car to enter garage XP values." />;

  const {
    activeKey, seedLoading,
    seedStageNums, getXp, updateXp,
    seedGarageLevelXp,
    isCorrectionMode, toggleCorrectionMode,
  } = fields;

  const correcting = isCorrectionMode(activeKey, 'xp');
  const xp = getXp(activeKey);
    ...
    <>
      ...
      <div className="StatsTabHeader">
        ...
        <label className="CorrectionToggle">
          <input type="checkbox" checked={correcting} onChange={() => toggleCorrectionMode('xp')} />
