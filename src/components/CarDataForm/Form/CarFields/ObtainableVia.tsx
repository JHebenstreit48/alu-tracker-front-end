...
export default function ObtainableVia({ ovEntries, setOvEntries }: Props) {
  ...
  const addOvEntry = () =>
    setOvEntries((prev) => [...prev, { id: nextId(), status: 'current', methods: [] }]);
  ...
  const addMethod = (id: number) => {
    ...
    setOvEntries((prev) =>
      prev.map((e) =>
        e.id === id && !e.methods.includes(val) ? { ...e, methods: [...e.methods, val] } : e
      )
    );
    inp.value = '';
    inp.focus();
    ...
    <div className="CarFieldsOvSection">
      ...
      <div className="CarFieldsOvList">
        {ovEntries.map((entry) => (
          <div
            key={entry.id}
            className={`OvEntry OvEntry--${entry.status}`}
          >
            ...
            <div className="OvEntry__addRow">
              <input
                ref={(el) => {
                  methodInputRefs.current[entry.id] = el;
                }}
