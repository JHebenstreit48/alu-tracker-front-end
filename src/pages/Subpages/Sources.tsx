...
export default function Sources() {
  ...
  const methodology = useMemo(
    () => [
      ...
      {
        title: "Why numbers may differ",
        body: [
          "Values may vary due to rounding, hidden decimals, version changes, and the game updating over time.",
          "The tracker aims to reflect the most recently verified state of the dataset.",
        ],
      },
    ],
    []
        ...
        <div className="SourcesPage__cards">
          {methodology.map((m) => (
            <article key={m.title} className="SourcesPage__card">
              ...
              {m.body.map((p) => (
                <p key={p} className="SourcesPage__cardText">
                  {p}
                </p>
              ))}
            </article>
      ...
      <section className="SourcesPage__section">
        ...
        {state === "ready" && derived.length > 0 ? (
          <ul className="SourcesPage__pillList">
            {derived.map((d) => (
              <li key={d.label} className="SourcesPage__pill">
                <span className="SourcesPage__pillLabel">{d.label}</span>
                <span className="SourcesPage__pillCount" aria-label={`Used ${d.count} times`}>
                  {d.count}
                </span>
              </li>
            ))}
